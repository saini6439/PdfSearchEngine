...
for file in glob.glob("corpus/*.pdf"):
    # print(file)
    string = str(file)
    pattern = '\\\+'
    result = re.split(pattern, string)
    # # print(str(result[1]))
    filename.append(result[1])
    # print(filename)
    nos_of_documents = nos_of_documents + 1;
...
def iterate_over_all_docs():
    ...
    for file in glob.glob("corpus/*.pdf"):
        ...
        token_list = get_tokenized_and_normalized_list(data)
        pdfname = file.split();
        print(pdfname)
        print("token list", token_list)
        vect = create_vector(token_list + pdfname)
        print("vect", vect)
        vects_for_docs.append(vect)
...
def get_result_from_query_vect(query_vector1):
    ...
    for i in range(nos_of_documents - 1):
        ...
        #print("I===========",i,"nos",nos_of_documents)
        parsed_list.append((filename[i], dot_prod))
